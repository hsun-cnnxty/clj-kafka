  ...
  (with-test-broker test-broker-config
    (with-resource [c (zk/consumer consumer-config)]
      ...
      (let [p (producer producer-config)]
        ...
        (let [offset-response (fetch-consumer-offsets "localhost:9999" "localhost:2182" "test" "clj-kafka.test.consumer")
              ignore (println offset-response)
  ...
  (with-test-broker test-broker-config
    (with-resource [c (zk/consumer consumer-config)]
      ...
      (let [p (producer producer-config)]
        ...
        (let [offset-reset-response (reset-consumer-offsets "localhost:9999" "localhost:2182" "test" "clj-kafka.test.consumer", :earliest)
